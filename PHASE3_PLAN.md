...
- **Phase 3d — Automation + cross-screen overlap view.** Scheduled refresh, a per-run validation report (missing columns, NaN-rate spikes, universe size changes), and a run-history log spanning every screen — folded together with the overlap view: `screen_membership` already has the data from 3a-3c, so the overlap view is largely a query plus a UI tab, not a phase of its own.
  - **Part 2c — Composition/misfile validation + per-screen `--force`. Done, 2026-09-01.** Driver ruling: heavy month-to-month ticker turnover is normal for these screens, so the universe-size-delta check (`check_universe_delta`, premised on a stable ticker count) was deleted outright — config keys, tests, everything — on all six screens, short_screen included. Replaced with `check_composition_misfile()`: a deliberately threshold-free check comparing an incoming export's tickers against *every* screen's stored baseline via Jaccard similarity, flagging only when some OTHER screen is a strictly better match than the screen's own — the real risk being an export landed in the wrong upload folder, not ordinary turnover. `refresh.py` gained `read_stored_ticker_sets()`, called once before the per-screen loop (not per-screen inside it) so every screen's comparison uses every peer's true pre-run state regardless of processing order — proven both by a regression test (`TestBaselineOrderIndependence`) and by a standalone script that deliberately read the baseline per-iteration and confirmed the verdict flips depending on order. Added per-screen `--force <screen_id>` (repeatable): findings are still computed and reported, but the write proceeds; `refresh_screen_runs` gained a `forced` 0/1 column (set once from the validation gate's outcome, unaffected by a later downstream transform/score failure). Live-data proof before the real run: pointing `competition` at `structural`'s upload folder (and the mirror) both correctly failed with `composition_misfile`, including on the thinnest measured pair (competition vs. structural, +0.039 margin) — see Known Issues in `CLAUDE.md`. The first real `refresh.py` run under the new check wrote 1,643 snapshot/membership rows across 1,375 unique tickers, all 6 screens PASSED with `forced=0`.
