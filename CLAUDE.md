...
- `src/transcript_ingest.py` — Phase 6a's Negative Expert Transcripts loader: explodes multi-ticker transcripts, synthesizes a missing `DocID`, and upserts the detail table so it accumulates across uploads (see the module docstring's DocID-backfill duplication hazard, also in Known Issues).
...
- **The composition/misfile check can also false-positive on a small incremental upload to an accumulating screen.** `negative_expert_transcripts`' `raw_data` table accumulates across uploads (see Rule 10's exception above), so `check_composition_misfile` Jaccards each new upload's ticker set against the WHOLE accumulated corpus, not just the prior upload — a small new batch's denominator is dominated by history, which can read as a worse match than an unrelated peer screen's baseline purely from set-size mismatch. Measured on this screen's own two source packs: incoming *Month Aug–Sep* (83 tickers) against a stored *Special Jul–Aug* baseline (101) passes at own Jaccard 0.064 vs. best peer `short_screen` 0.053 — a +0.011 margin, the thinnest in the project (thinner than competition-vs-structural's +0.039 above) — and a mild variation (a pack sharing a few names with `structural`, none with its own corpus) flips it to a flag (own 0.003 vs. structural 0.019). The two packs in this file share only 11 of 173 tickers, so low recurrence is the normal regime. Not fixed — `src/validate.py` is unscoped for Phase 6a; full derivation in `PHASE6_SCOPE.md` §7.
...
- **A `negative_expert_transcripts` DocID backfill can duplicate transcripts.** 14 of the initial vintage's 215 transcripts have no source `DocID` and are stored under a synthesized `SYN-<hash>` id (`src/transcript_ingest.py`'s `synthesize_doc_ids`). If the same period is ever re-exported with `DocID` now populated (the Driver has agreed to backfill it going forward), those 14 rows arrive under different, real ids, don't match the `(doc_id, ticker)` key of the already-stored `SYN-` rows, and are inserted IN ADDITION to them — duplicating those transcripts and inflating `mention_count`. Detection (flagging a `(transcript_date, theme, ticker)` resolving to more than one `doc_id`) is deliberately out of scope for 6a. **Operational rule until that detection exists: re-ingesting a period already stored requires clearing this screen's `raw_data` table first** — the upload files kept in `data/uploads/negative_expert_transcripts/_archive/` make the corpus rebuildable, which is what makes that safe.
