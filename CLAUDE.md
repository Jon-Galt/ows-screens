...
- **Phase 5e** — export buttons: `st.columns([1, 1, 8])` replaced by `st.container(horizontal=True, gap=16)` at all four export sites, labels shortened to "Excel"/"CSV" with a shared `:material/download:` glyph wired through `EXPORT_BUTTON_ICON` so its validity is locked by import rather than by re-typing. The column ratio had made button width proportional to the viewport and truncated both labels to an indistinguishable "Export ...". Shipped `bfe7404`.
...
- **Phase 6c** — the cross-screen seam: the transcripts contribution in "Also Appears On" (`mention_count` / `latest_transcript_date` / `days_since_latest`), `mention_count` joined onto the Cross-Screen Overlap view through `OVERLAP_METRIC_JOINS`, that table's live figures **derived** by `build_overlap_help_map` instead of hand-typed, and `format_unscored_metric_value()` extracted so both unscored render sites share one guard (T42). Retired `_UNSCORED_METRIC_COLUMNS` as the last RSI-shaped default. Shipped `85249d8`.
...
- `src/cross_screen_context.py` — `classify_screen()` (the single screen taxonomy both `app.py` loaders dispatch through), `build_screen_contribution()`, `build_also_appears_on()`. Identity is never repeated per screen. Phase 8b added `is_transcript_shaped()`, the column-set gate for the transcripts takeaways block (T43).
