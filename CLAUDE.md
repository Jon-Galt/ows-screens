...
- **PM → Worker:** every Worker prompt and every revision prompt, in the chat as ONE block, and
  ...
  deliverable. A digest, or "it's in the file," is not a delivery.
...
- **PM → Tom (Driver):** discussion, review verdicts and the reasoning behind them, findings,
  ...
  because the previous wording told the PM to block these and it did.)
...
- [ ] Output is ONE four-backtick fenced block per Output format above — plan, build report, review, AND the closing summary of an execution turn, including a turn that only applied corrections or only committed. Numbered "report back" results go INSIDE the block.
